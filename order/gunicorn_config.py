...
def post_fork(server, worker):
    ...
    worker.consumer = OrderConsumer(worker.db)
...
def worker_exit(server, worker):
    ...
    # Close the producer connection if needed
    if hasattr(worker.app, "producer"):
        # Add a close method to your OrderProducer class if needed
        if hasattr(worker.app.producer, "close"):
            worker.app.producer.close()
