...
class PaymentProducer:
    ...
    async def init(self):
        ...
        try:
            ...
            app.logger.info("Successfully connected to RabbitMQ (PaymentProducer)")
        except Exception as e:
            app.logger.error(f"Failed to connect to RabbitMQ (PaymentProducer): {e}")
    ...
    async def send_message(self, routing_key, event_data):
        ...
        try:
            ...
            app.logger.debug(
                f"Message sent to exchange 'payment.exchange' with key '{routing_key}': {message_body}"
            )
        except Exception as e:
            error_context = (
    ...
    async def close(self):
        if self.connection and not self.connection.is_closed:
            await self.connection.close()
            current_app.logger.info("PaymentProducer: RabbitMQ connection closed")
